...
const typeConfig = {
  feature: {
    ...
    color: 'bg-secondary-500/[0.16] text-secondary-300 border-secondary-500/30',
    icon: (
  ...
  fix: {
    label: 'Bug Fix',
    color: 'bg-danger-500/[0.16] text-danger-300 border-danger-500/30',
    icon: (
      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  improvement: {
    label: 'Miglioramento',
    color: 'bg-primary-500/[0.16] text-primary-300 border-primary-500/30',
    icon: (
      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className="min-h-screen">
        ...
        <div className="mb-5">
          <div className="flex items-center justify-between gap-4 mb-3">
            <div className="flex items-center gap-3">
              <div className="w-[42px] h-[42px] rounded-xl bg-gradient-to-br from-primary-800 to-primary-500 flex items-center justify-center shadow-glow">
                <svg className="w-[21px] h-[21px] text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                </svg>
              </div>
              <div>
                <h1 className="font-display text-lg sm:text-xl font-bold text-white leading-tight">Feedback Hub</h1>
                <p className="text-sm text-gray-400">Novita, segnalazioni e suggerimenti</p>
              </div>
            </div>
            <button
              onClick={() => { setShowForm(true); }}
              className="font-display px-4 py-2.5 bg-primary-500 hover:bg-primary-600 text-white font-bold text-sm rounded-xl transition-colors flex items-center gap-2 shadow-glow flex-shrink-0"
            >
          ...
          {/* Alpha Test Badge (warning/amber, invariato) */}
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-accent-500/10 border border-accent-500/30 rounded-lg">
            <div className="w-2 h-2 bg-accent-500 rounded-full shadow-glow-gold animate-pulse" />
            <span className="text-xs text-accent-400 font-semibold">Alpha Test</span>
            <span className="text-xs text-gray-500">Aiutaci a migliorare segnalando problemi!</span>
          </div>
        </div>

        {/* Tabs (PanelTabs style: sottolineatura oro) */}
        <div className="flex gap-6 mb-5 border-b border-surface-50">
          {([
            { key: 'news', label: 'Novità' },
            { key: 'my-feedback', label: 'Le mie Segnalazioni' },
            ...(isSuperAdmin ? [{ key: 'all-feedback', label: 'Tutte le Segnalazioni' }] : []),
            ...
            return (
              <button
                ...
                className={`relative font-display text-sm font-semibold pb-3 pt-2.5 transition-colors ${
                  isActive ? 'text-white' : 'text-gray-400 hover:text-white'
                }`}
              >
                {tab.label}
                {isActive && (
                  <span className="absolute left-0 right-0 -bottom-px h-0.5 bg-accent-500 rounded-sm shadow-glow-gold" />
                )}
              </button>
            )
          })}
          ...
          <div className="space-y-3.5">
            {PATCH_NOTES.map((patch) => {
              ...
              return (
                <div
                  key={patch.id}
                  className="bg-surface-200 rounded-2xl border border-surface-50 overflow-hidden hover:border-primary-500/35 transition-colors"
                >
                  {/* Header */}
                  <div className="px-4 py-3 bg-surface-300 border-b border-surface-50 flex items-center justify-between flex-wrap gap-2.5">
                    <div className="flex items-center gap-2.5">
                      {/* Version Badge */}
                      <span className="px-2.5 py-1 bg-primary-500/[0.16] text-primary-300 text-xs font-mono font-bold rounded-lg border border-primary-500/30">
                        {patch.version}
                      </span>

                      {/* Type Badge (semantico) */}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface-200 rounded-2xl border border-surface-50 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-card-hover">
            <div className="px-6 py-4 border-b border-surface-50 flex items-center justify-between">
              <h2 className="font-display text-base font-bold text-white">Nuova Segnalazione</h2>
              <button
                onClick={() => { setShowForm(false); }}
                className="w-8 h-8 border border-surface-50 bg-surface-300 text-gray-400 hover:text-white hover:bg-surface-100 rounded-lg flex items-center justify-center transition-colors"
              >
