                ...
                <div className="px-4 pb-4 border-t border-surface-50/20">
                  ...
                  <div className="md:hidden space-y-3">
                    {session.members.map(member => (
                      <div key={member.memberId} className="bg-surface-300/30 rounded-lg p-3 border border-surface-50/10">
                        {/* Header: Manager + Total */}
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="text-right">
                            ...
                            <div className="text-xl font-bold text-yellow-400">{member.total}M</div>
                          </div>
                        </div>
                        {/* Prizes Grid */}
                        <div className="grid grid-cols-3 gap-2 text-center text-xs">
                          <div className="bg-surface-300/50 rounded p-1.5">
                            <div className="text-gray-500 text-[10px] uppercase">Base</div>
                            <div className="text-gray-300 font-medium">{member.baseReincrement}M</div>
                          </div>
                          {session.categories.map(cat => (
                            <div key={cat.name} className="bg-surface-300/50 rounded p-1.5">
                              <div className="text-gray-500 text-[10px] uppercase truncate" title={cat.name}>
                                {cat.name.length > 8 ? cat.name.substring(0, 8) + '..' : cat.name}
                              </div>
                              <div className="text-gray-300 font-medium">{member.categoryPrizes[cat.name] ?? 0}M</div>
                            </div>
                          ))}
                        </div>
                      </div>
