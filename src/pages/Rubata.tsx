...
function PreferenceModal({
  ...
  const _hasStrategy = formData.maxBid || formData.priority || formData.notes
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              {player.playerTeam} • {player.rubataPrice}M
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 text-center">
                <input
                  ...
                  onChange={e => {
                    setFormData(p => ({ ...p, maxBid: e.target.value }))
                  }}
                  placeholder="—"
          ...
          <div>
            ...
            <p className="text-center text-xs text-gray-500 mt-1">
              {formData.priority
                ? `Priorità ${formData.priority} (clicca per rimuovere)`
...
export function Rubata({ leagueId, onNavigate }: RubataProps) {
  ...
  const [selectedPlayerForPrefs, setSelectedPlayerForPrefs] =
    useState<BoardPlayerWithPreference | null>(null)
        ...
        setBoardData(prev => {
          ...
          return {
            ...
            activeAuction: {
              ...
              bids: [newBid, ...prev.activeAuction.bids.map(b => ({ ...b, isWinning: false }))],
            },
          }
        })
      }
  ...
  useEffect(() => {
    const getPollingInterval = () => {
      ...
      if (isPusherConnected) {
        if (state === 'AUCTION') return 10000 // Fallback during auction
        if (state === 'AUCTION_READY_CHECK') return 10000
        if (state === 'OFFERING') return 10000
        return 15000 // Long fallback for other states
      }

      // Without Pusher, use shorter intervals
      if (state === 'AUCTION') return 5000
      if (state === 'AUCTION_READY_CHECK') return 5000
      if (state === 'OFFERING') return 5000
      return 5000
    }
  ...
  }, [loadBoardOnly, loadFast, loadAckOnly, loadData, boardData?.rubataState, isPusherConnected])
  ...
  async function handleCompleteRubata() {
    if (!confirm('Vuoi completare la rubata con transazioni casuali? Questo è irreversibile.'))
      return
    setError('')
    setIsSubmitting(true)
    ...
    setBoardData(prev => {
      ...
      return {
        ...
        activeAuction: {
          ...
          bids: [optimisticBid, ...prev.activeAuction.bids.map(b => ({ ...b, isWinning: false }))],
        },
      }
    })
  ...
  async function _handleAcknowledge() {
    setError('')
    setIsSubmitting(true)
  ...
  async function _handleSetToPreview() {
    setError('')
    setIsSubmitting(true)
  ...
  async function handleSavePreference(data: {
    ...
    const hasStrategy =
      data.maxBid !== null || data.priority !== null || !!(data.notes && data.notes.trim() !== '')
  ...
  const canMakeOffer =
    rubataState === 'OFFERING' && currentPlayer && currentPlayer.memberId !== myMemberId
  ...
  return (
    <div className="min-h-screen bg-dark-300">
            ...
            <div className="bg-gradient-to-br from-purple-900 to-purple-950 rounded-3xl p-6 max-w-lg w-full shadow-2xl border-2 border-purple-400 animate-bounce-in max-h-[90vh] overflow-y-auto">
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-purple-200 text-sm mt-1">
                  Conferma la transazione per procedere
                </p>
              </div>
              ...
              <div className="bg-white/10 rounded-2xl p-4 mb-4">
                <div className="flex items-center justify-center gap-3 mb-3">
                  ...
                  <div
                    className={`w-10 h-10 flex items-center justify-center rounded-full ${POSITION_COLORS[pendingAck.player.position] || 'bg-gray-500/20 text-gray-400'} border-2`}
                  >
                    <span className="font-bold">{pendingAck.player.position}</span>
                  </div>
                </div>
                <p className="text-center text-2xl font-bold text-white">
                  {pendingAck.player.name}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                ...
                <div className="bg-white/5 rounded-xl p-3 text-center">
                  ...
                  <p className="font-bold text-secondary-400">
                    {pendingAck.winner?.username || 'Nessuno'}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white/5 rounded-xl p-4 mb-4">
                ...
                <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto">
                  {pendingAck.acknowledgedMembers.map(member => (
                    <div
                      key={member.id}
                      className="flex items-center gap-2 p-2 rounded-lg bg-secondary-500/20 text-sm"
                    >
              ...
              <div className="space-y-2">
                {!isAppealMode ? (
                  <>
                    ...
                    >
                      {prophecyContent.trim()
                        ? '🔮 CONFERMA CON PROFEZIA'
                ...
                {isAdmin && (
                  <div className="pt-2 border-t border-white/10 space-y-2">
                    ...
                    <Button
                      ...
                      disabled={isSubmitting}
                      variant="outline"
                      className="w-full text-xs border-danger-500/50 text-danger-400"
                    >
                      🤖 [TEST] Simula ricorso di un DG
                    </Button>
                    <Button
                      onClick={handleForceAllAcknowledge}
                      disabled={isSubmitting}
                      variant="outline"
          ...
          <div className="bg-gradient-to-br from-danger-900 to-danger-950 rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto border-2 border-danger-500">
            <div className="p-6">
              <div className="text-center mb-6">
                ...
                <p className="text-danger-200 mt-1">
                  La transazione è sospesa in attesa della decisione dell'admin
                </p>
              </div>
              ...
              {appealStatus?.player && (
                <div className="bg-white/10 rounded-xl p-4 mb-4 flex items-center gap-3">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${POSITION_COLORS[appealStatus.player.position] || 'bg-gray-500/20 text-gray-400 border-gray-500/30'}`}
                  >
              ...
              {appealStatus?.appeal && (
                <div className="bg-danger-500/10 border border-danger-500/30 rounded-xl p-4 mb-4">
                  <p className="text-xs text-danger-300 uppercase font-bold mb-2">
                    Motivo del ricorso
                  </p>
              ...
              {appealStatus?.winner && (
                <div className="bg-white/10 rounded-xl p-4 mb-4 text-center">
                  ...
                  <p className="text-2xl font-black text-accent-400 mt-1">
                    {appealStatus.finalPrice}M
                  </p>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
          <div className="bg-gradient-to-br from-orange-900 to-orange-950 rounded-3xl p-6 max-w-lg w-full shadow-2xl border-4 border-orange-400 animate-bounce-in">
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="text-3xl font-black text-white uppercase tracking-wide">RUBATA!</h2>
              <p className="text-orange-200 text-sm mt-2">
                Qualcuno vuole rubare questo giocatore!
              </p>
            </div>
            ...
            <div className="bg-white/10 rounded-2xl p-4 mb-4">
              <div className="flex items-center justify-center gap-3 mb-2">
                ...
                <div
                  className={`w-10 h-10 flex items-center justify-center rounded-full ${POSITION_COLORS[boardData.auctionReadyInfo.playerPosition] || 'bg-gray-500/20 text-gray-400'} border-2`}
                >
              ...
              <p className="text-center text-orange-200">
                ...
                <span className="font-semibold text-white">
                  {boardData.auctionReadyInfo.ownerUsername}
                </span>
              </p>
            </div>
            ...
            <div className="text-center mb-4">
              <p className="text-orange-200 text-sm uppercase tracking-wider mb-1">Prezzo rubata</p>
      ...
      <main className="max-w-[1600px] mx-auto px-4 py-8">
        ...
        {success && (
          <div className="bg-secondary-500/20 border border-secondary-500/30 text-secondary-400 p-3 rounded-lg mb-4">
            {success}
          </div>
        )}
        ...
        {!isRubataPhase && (
          <div className="bg-surface-200 rounded-2xl border border-surface-50/20 p-8 text-center">
            ...
            <p className="text-gray-400 max-w-lg mx-auto">
              La fase rubata inizierà dopo il consolidamento dei contratti. Attendi che l'admin
              della lega passi alla fase RUBATA.
            </p>
          </div>
        )}
            ...
            <div className="bg-surface-200 rounded-2xl border border-surface-50/20 overflow-hidden">
              <div className="p-5 border-b border-surface-50/20">
                ...
                <p className="text-sm text-gray-400 mt-1">
                  Trascina i manager per impostare l'ordine dei turni
                </p>
              </div>
              <div className="p-5">
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-white font-medium">
                            {member?.user?.username || member?.teamName || 'Unknown'}
                          </span>
                        </div>
                        <div className="flex gap-1">
                          <button
                            onClick={() => {
                              moveInOrder(index, 'up')
                            }}
                            disabled={index === 0}
              ...
              <div className="p-5 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-400 mb-2">
                    Timer offerta iniziale (secondi)
                  </label>
                  <input
                    type="number"
                    value={offerTimer}
                    onChange={e => {
                      setOfferTimer(parseInt(e.target.value) || 30)
                    }}
                    min={5}
                    max={120}
                ...
                <div>
                  ...
                  <input
                    ...
                    value={auctionTimer}
                    onChange={e => {
                      setAuctionTimer(parseInt(e.target.value) || 15)
                    }}
                    min={5}
                    max={60}
                    className="w-full px-4 py-2 bg-surface-300 border border-surface-50/30 rounded-xl text-white focus:border-primary-500 focus:outline-none"
        ...
        {isRubataPhase && !isOrderSet && !isAdmin && (
          <div className="bg-surface-200 rounded-2xl border border-surface-50/20 p-8 text-center">
            ...
            <p className="text-gray-400 max-w-lg mx-auto">
              L'admin della lega sta impostando l'ordine di rubata. Una volta confermato, potrai
              vedere il tabellone e partecipare alle aste.
            </p>
          </div>
        )}
        ...
        {isRubataPhase && isOrderSet && (
          <div className={isAdmin ? 'grid grid-cols-1 lg:grid-cols-5 gap-6' : ''}>
                  ...
                  <div className="p-3 space-y-3">
                    <div>
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => {
                            setOfferTimer(prev => Math.max(5, prev - 5))
                          }}
                        ...
                        <input
                          ...
                          value={offerTimer}
                          onChange={e => {
                            setOfferTimer(Math.max(5, parseInt(e.target.value) || 5))
                          }}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => {
                            setAuctionTimer(prev => Math.max(5, prev - 5))
                          }}
                          className="w-10 h-10 shrink-0 rounded-lg bg-surface-300 text-white hover:bg-surface-100 text-xl font-bold flex items-center justify-center"
                        >
                          −
                        ...
                        <input
                          ...
                          value={auctionTimer}
                          onChange={e => {
                            setAuctionTimer(Math.max(5, parseInt(e.target.value) || 5))
                          }}
                          className="w-full min-w-0 text-center bg-surface-300 border border-surface-50/30 rounded-lg py-2 text-white text-lg font-bold"
                        />
                        <button
                          onClick={() => {
                            setAuctionTimer(prev => prev + 5)
                          }}
                          className="w-10 h-10 shrink-0 rounded-lg bg-surface-300 text-white hover:bg-surface-100 text-xl font-bold flex items-center justify-center"
                        >
                          +
                    ...
                    <div className="p-4 space-y-3">
                      <div>
                        ...
                        <select
                          ...
                          onChange={e => {
                            setSimulateMemberId(e.target.value)
                          }}
                        ...
                        <>
                          <div>
                            ...
                            <div className="flex items-center gap-1">
                              <button
                                onClick={() => {
                                  setSimulateBidAmount(prev =>
                                    Math.max(activeAuction.currentPrice + 1, prev - 1)
                                  )
                                }}
                              ...
                              <button
                                onClick={() => {
                                  setSimulateBidAmount(prev => prev + 1)
                                }}
                                className="w-8 h-8 rounded-lg bg-surface-300 text-white text-sm"
                              >
                                +
                          ...
                          <Button
                            ...
                            disabled={
                              isSubmitting ||
                              !simulateMemberId ||
                              simulateBidAmount <= activeAuction.currentPrice
                            }
                            size="sm"
                            variant="outline"
                            className="w-full border-orange-500/50 text-orange-400"
                ...
                <div className="bg-surface-200 rounded-2xl border border-danger-500/50 overflow-hidden">
                  ...
                  <div className="p-3">
                    <p className="text-xs text-gray-400 mb-3">
                      Completa la rubata con transazioni casuali (30% rubate)
                    </p>
                    <Button
            ...
            <div className={isAdmin ? 'lg:col-span-4' : ''}>
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4">
                      {currentPlayer ? (
                        <>
                          <div
                            className={`w-12 h-12 rounded-full ${POSITION_COLORS[currentPlayer.playerPosition]} border flex items-center justify-center font-bold text-lg`}
                          >
                          ...
                          <div>
                            ...
                            <p className="text-gray-400">
                              {currentPlayer.playerTeam} • {currentPlayer.ownerUsername}
                            </p>
                          </div>
                          <div className="ml-4 text-right">
                            <p className="text-2xl font-bold text-primary-400">
                              {currentPlayer.rubataPrice}M
                            </p>
                          ...
                          {currentPlayerPreference && currentPlayer.memberId !== myMemberId && (
                            <div className="ml-4 px-3 py-2 bg-indigo-500/20 border border-indigo-500/40 rounded-lg">
                              <p className="text-[10px] text-indigo-300 uppercase mb-1">
                                La tua strategia
                              </p>
                              <div className="flex items-center gap-2 text-sm">
                                {currentPlayerPreference.isWatchlist && (
                                  <span title="Watchlist">⭐</span>
                                )}
                                {currentPlayerPreference.isAutoPass && (
                                  <span title="Auto-pass">🚫</span>
                                )}
                                {currentPlayerPreference.priority && (
                                  <span className="text-purple-400">
                                    {'★'.repeat(currentPlayerPreference.priority)}
                                  </span>
                                )}
                                {currentPlayerPreference.maxBid && (
                                  <span className="text-blue-400">
                                    Max: {currentPlayerPreference.maxBid}M
                                  </span>
                                )}
                                {currentPlayerPreference.notes && (
                                  <span
                                    className="text-gray-400 truncate max-w-[100px]"
                                    title={currentPlayerPreference.notes}
                                  >
                                    📝
                                  </span>
                                )}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div
                        ...
                        title={isPusherConnected ? 'Real-time connesso' : 'Real-time disconnesso'}
                      >
                        <div
                          className={`w-2 h-2 rounded-full ${isPusherConnected ? 'bg-secondary-400' : 'bg-danger-400 animate-pulse'}`}
                        />
                        <span
                          className={`text-[10px] uppercase tracking-wider ${isPusherConnected ? 'text-secondary-400' : 'text-danger-400'}`}
                        >
                      ...
                      {timerDisplay !== null && (
                        <div
                          className={`text-4xl font-mono font-bold ${timerDisplay <= 5 ? 'text-danger-400 animate-pulse' : timerDisplay <= 10 ? 'text-warning-400' : 'text-white'}`}
                        >
                          {timerDisplay}s
                        </div>
                      )}
                          ...
                          <div className="flex flex-wrap gap-2 mt-2 text-xs">
                            {progressStats.managerProgress && (
                              <span className="px-2 py-0.5 bg-primary-500/20 rounded text-primary-400">
                                👤 {progressStats.managerProgress.username}:{' '}
                                {progressStats.managerProgress.processed}/
                                {progressStats.managerProgress.total}
                              </span>
                            )}
                            <span className="px-2 py-0.5 bg-accent-500/20 rounded text-accent-400">
                              📊 Totale: {progressStats.currentIndex + 1}/
                              {progressStats.totalPlayers}
                            </span>
                      ...
                      {(rubataState === 'OFFERING' || rubataState === 'AUCTION') && (
                        <>
                          ...
                          <Button
                            ...
                            disabled={isSubmitting || boardData?.currentIndex === 0}
                            variant="outline"
                          >
                            ⏮️ Indietro
                          </Button>
                          ...
                          <div className="flex flex-wrap gap-3 items-center">
                            {currentPlayerPreference.maxBid && (
                              <div className="bg-black/20 rounded-lg px-3 py-1.5">
                                <span className="text-[10px] text-indigo-300 uppercase mr-1">
                                  Max:
                                </span>
                                <span className="font-bold text-blue-400">
                                  {currentPlayerPreference.maxBid}M
                                </span>
                              </div>
                            )}
                            {currentPlayerPreference.priority && (
                  ...
                  {readyStatus.pendingMembers.length > 0 && (
                    <div className="flex items-center gap-2 flex-wrap text-sm">
                      ...
                      {readyStatus.pendingMembers.map((member, _idx) => (
                        <span
                          key={member.id}
                          className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-warning-500/20 text-warning-400 rounded text-xs"
                        >
                          <span
              ...
              {rubataState === 'PAUSED' && readyStatus && (
                <div className="mb-4 bg-surface-200 rounded-xl border border-gray-500/50 p-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {boardData?.pausedRemainingSeconds !== null &&
                          boardData.pausedRemainingSeconds !== undefined && (
                            <span className="text-yellow-400 text-sm ml-2">
                              ({boardData.pausedRemainingSeconds}s rimanenti -{' '}
                              {boardData.pausedFromState === 'AUCTION' ? 'Asta' : 'Offerta'})
                            </span>
                          )}
                      </div>
                    </div>
                  ...
                  <p className="text-gray-400 text-xs text-center">
                    L'admin ha messo in pausa la rubata. Tutti i manager devono confermare di essere
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div className="flex-1 text-center">
                                ...
                                <p className="text-xs text-gray-500">
                                  Min: {activeAuction.currentPrice + 1}M
                                </p>
                              </div>
                              <button
                                onClick={() => {
                          ...
                          {activeAuction.bids.length === 0 ? (
                            ...
                            activeAuction.bids.map((bid, i) => (
                              ...
                              >
                                ...
                                <span className="ml-2 font-mono text-primary-400">
                                  {bid.amount}M
                                </span>
                                {bid.isWinning && (
                                  <span className="ml-2 text-secondary-400 text-sm font-medium">
                                    ✓ Vincente
                                  </span>
                                )}
                              </div>
              ...
              {/* Tabellone completo */}
              <div
                className="bg-surface-200 rounded-2xl border border-surface-50/20 overflow-hidden flex flex-col"
              ...
              >
                <div className="p-5 border-b border-surface-50/20 shrink-0">
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    {boardData?.totalPlayers} giocatori in ordine di rubata
                  </p>
                </div>
                      ...
                      {board?.map((player, globalIndex) => {
                        ...
                        return (
                          <tr
                            ...
                            ref={
                              isCurrent
                                ? (currentPlayerRef as React.RefObject<HTMLTableRowElement>)
                                : null
                            }
                          ...
                          >
                            <td className="p-2 font-mono text-xs">
                              {isCurrent ? (
                                ...
                                <span className={isPassed ? 'text-gray-600' : 'text-gray-500'}>
                                  {globalIndex + 1}
                                </span>
                              )}
                            </td>
                            <td className="p-2">
                              <div className="flex items-center gap-1">
                                ...
                                <div
                                  className={`w-5 h-5 flex items-center justify-center rounded text-[10px] ${POSITION_COLORS[player.playerPosition]}`}
                                >
                                  <span className="font-bold">{player.playerPosition}</span>
                                </div>
                                <span
                                  className={`font-medium truncate ${isCurrent ? 'text-white font-bold' : isPassed ? 'text-gray-500' : 'text-gray-300'}`}
                                >
                                  {player.playerName}
                                </span>
                              </div>
                            ...
                            <td className="p-2">
                              <span
                                className={`truncate block ${isPassed && wasStolen ? 'text-gray-500 line-through' : isPassed ? 'text-gray-500' : 'text-gray-400'}`}
                              >
                            ...
                            <td className="p-2 text-center">
                              <span
                                className={`text-xs ${isCurrent ? 'text-accent-400' : isPassed ? 'text-gray-600' : 'text-accent-400'}`}
                              >
                            ...
                            <td className="p-2 text-center">
                              <span
                                className={`text-xs font-medium ${
                                  isPassed
                                    ? 'text-gray-500'
                                    : player.contractDuration === 1
                                      ? 'text-danger-400'
                                      : player.contractDuration === 2
                                        ? 'text-warning-400'
                                        : player.contractDuration === 3
                                          ? 'text-blue-400'
                                          : 'text-secondary-400'
                                }`}
                              >
                            ...
                            <td className="p-2 text-center">
                              <span
                                className={`text-xs ${isPassed ? 'text-gray-600' : 'text-gray-400'}`}
                              >
                            ...
                            <td className="p-2 text-center">
                              <span
                                className={`text-xs font-bold ${isCurrent ? 'text-primary-400' : isPassed ? 'text-gray-600' : 'text-warning-400'}`}
                              >
                                {player.rubataPrice}
                              </span>
                            </td>
                            ...
                            <td className="p-2 text-center">
                              {(() => {
                                ...
                                if (isMyPlayer)
                                  return <span className="text-gray-600 text-xs">Mio</span>
                                  ...
                                  <div className="flex items-center justify-center gap-1">
                                    ...
                                    {pref?.maxBid && (
                                      <span
                                        ...
                                        title={`Max ${pref.maxBid}M`}
                                      >
                                        {pref.maxBid}M
                                      </span>
                                    )}
                                    {pref?.notes && !pref.priority && !pref.maxBid && (
                                      <span className="text-gray-400 text-xs" title={pref.notes}>
                                        📝
                                      </span>
                                    )}
                                    {/* Edit button */}
                                    {canEditPreferences && (
                                      <button
                                        type="button"
                                        onClick={() => {
                                          openPrefsModal({ ...player, preference: pref || null })
                                        }}
                                        className={`w-6 h-6 rounded flex items-center justify-center text-xs transition-all ${
                                          hasStrategy
                                            ? 'bg-indigo-500/30 text-indigo-400'
                                            : 'bg-surface-50/20 text-gray-500 hover:bg-indigo-500/20'
                                        }`}
                  ...
                  {board?.map((player, globalIndex) => {
                    ...
                    return (
                      <div
                        key={player.rosterId}
                        ref={
                          isCurrent ? (currentPlayerRef as React.RefObject<HTMLDivElement>) : null
                        }
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          {isCurrent ? (
                            ...
                            <span
                              className={`text-xs font-mono w-6 text-center ${isPassed ? 'text-gray-600' : 'text-gray-500'}`}
                            >
                              #{globalIndex + 1}
                            </span>
                          )}
                          ...
                          <div
                            className={`w-6 h-6 flex items-center justify-center rounded ${POSITION_COLORS[player.playerPosition]}`}
                          >
                            <span className="text-xs font-bold">{player.playerPosition}</span>
                          </div>
                          <span
                            className={`font-medium flex-1 truncate ${isCurrent ? 'text-white font-bold' : isPassed ? 'text-gray-500' : 'text-gray-300'}`}
                          >
                            {player.playerName}
                          </span>
                          {isCurrent && (
                        ...
                        <div className="text-xs text-gray-500 mb-2 pl-6">
                          ...
                          <span
                            className={
                              isPassed && wasStolen ? 'text-gray-500 line-through' : 'text-gray-400'
                            }
                          >
                            {player.ownerUsername}
                          </span>
                          {player.ownerTeamName && (
                            <span className="text-gray-600"> ({player.ownerTeamName})</span>
                          )}
                        </div>
                        ...
                        {/* Dettagli contratto */}
                        <div
                          className={`grid grid-cols-4 gap-2 rounded p-2 ${isPassed ? 'bg-surface-50/5' : 'bg-surface-50/10'}`}
                        >
                          <div className="text-center">
                            <div className="text-[10px] text-gray-500 uppercase">Ingaggio</div>
                            <div
                              className={`font-medium text-sm ${isPassed ? 'text-gray-600' : 'text-accent-400'}`}
                            >
                          ...
                          <div className="text-center">
                            ...
                            <div
                              className={`font-medium text-sm ${isPassed ? 'text-gray-600' : 'text-gray-400'}`}
                            >
                          ...
                          <div className="text-center">
                            ...
                            <div
                              className={`font-bold text-sm ${isPassed ? 'text-gray-600' : isCurrent ? 'text-primary-400' : 'text-warning-400'}`}
                            >
        ...
        {isRubataPhase && isOrderSet && sessionId && (
          <div className="fixed bottom-4 right-4 z-40 w-80">
            <Chat sessionId={sessionId} currentMemberId={boardData?.myMemberId} isAdmin={isAdmin} />
          </div>
        )}
      </main>
