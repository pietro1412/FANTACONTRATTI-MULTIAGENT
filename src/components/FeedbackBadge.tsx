  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        {/* Badge counter */}
        {count > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] flex items-center justify-center px-1 text-[10px] font-bold text-white bg-primary-500 rounded-full shadow-lg shadow-primary-500/40 animate-pulse">
            {count > 9 ? '9+' : count}
          </span>
        )}
      </button>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-surface-200 border border-surface-50/30 rounded-xl shadow-2xl shadow-black/40 overflow-hidden z-50">
          {/* Header */}
          <div className="px-4 py-3 bg-surface-300 border-b border-surface-50">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4 text-primary-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                </svg>
                <span className="font-display text-sm font-bold text-white">Segnalazioni</span>
              </div>
              {count > 0 && (
                <button
            ...
            {isLoading ? (
              ...
              <div className="py-2">
                {notifications.map(notification => {
                  const statusCfg = statusConfig[notification.feedbackStatus] ?? { label: 'Aperta', color: 'bg-accent-500/20 text-accent-400' }
          ...
          {/* Footer */}
          <div className="px-4 py-2 bg-surface-300 border-t border-surface-50">
            <button
              onClick={() => {
                setIsOpen(false)
                onNavigate('feedbackHub')
              }}
              className="w-full px-3 py-1.5 text-xs font-semibold text-primary-300 hover:text-primary-200 hover:bg-primary-500/10 rounded-lg transition-colors flex items-center justify-center gap-1.5"
            >
