...
export const HeroPlayerCard = memo(function HeroPlayerCard({
  ...
  const showCTA = rubataState === 'OFFERING' && canMakeOffer && !isMyPlayer
  // Arena ORIZZONTALE su desktop solo quando la colonna destra ha contenuto di decisione
  const hasRightColumn = showAuctionBox || showAffordability || showCTA || !!children
  ...
  return (
    ...
    >
      <div className="p-4">
        ...
        {/* P2: arena orizzontale — identità+costo a sinistra · decisione a destra (desktop) */}
        <div className={hasRightColumn ? 'lg:grid lg:grid-cols-[1fr_1.05fr]' : ''}>

          {/* ===== Colonna sinistra: identità + costo + intel ===== */}
          <div className={hasRightColumn ? 'lg:pr-4 lg:min-w-0' : ''}>
            {/* Player head */}
            ...
            <div className="mt-3 flex items-center gap-4 flex-wrap rounded-xl border border-accent-500/40 bg-surface-300 px-4 py-3">
              <div className="flex flex-col">
                ...
                <span className="stat-number text-[44px] sm:text-[54px] lg:text-[40px] leading-none text-accent-300">{player.rubataPrice}M</span>
              </div>
            ...
            {!isMyPlayer && canEditPreferences && (
              <div className="mt-2.5 flex justify-end lg:justify-start">
                <button
                  type="button"
            ...
            <div className="lg:border-l lg:border-surface-50 lg:pl-4 lg:min-w-0 lg:flex lg:flex-col lg:gap-2.5">
              ...
              {showCTA && (
                <button
                  ...
                  className="mt-3 lg:mt-0 w-full py-3.5 rounded-xl font-display font-extrabold text-lg uppercase tracking-wide text-dark-300 bg-gradient-to-b from-secondary-400 to-secondary-500 hover:from-secondary-300 hover:to-secondary-400 shadow-glow-green transition-all active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed"
                >
